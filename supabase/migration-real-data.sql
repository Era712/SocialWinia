alter table public.giveaways
  ...
  add column if not exists prize_value_chf numeric default 0,
  add column if not exists winner_count integer default 1,
  add column if not exists processing_method text default 'openai',
...
begin
  if exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'prize_value_usd'
  ) then
    execute 'update public.giveaways set prize_value_chf = coalesce(prize_value_chf, prize_value_usd, 0) where prize_value_chf is null';
  end if;
end $$;
